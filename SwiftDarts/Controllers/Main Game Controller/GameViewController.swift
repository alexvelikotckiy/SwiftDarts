...
class GameViewController: UIViewController {
    ...
    enum SessionState {
        ...
        var localizedInstruction: String? {
            ...
            switch self {
            case .lookingForSurface:
                return NSLocalizedString("Find a flat vertical surface to place the game.", comment: "")
            case .placingBoard:
    ...
    @IBOutlet weak var teamAPointsLabel: UILabel!
    @IBOutlet weak var teamBPointsLabel: UILabel!
    
    // MARK: - Var & let
    private var teamAScoresCount = 0 {
        didSet {
            guard oldValue != teamAScoresCount else { return }
            
            teamAPointsLabel.text = String(teamAScoresCount)
        }
    }
    
    private var teamBScoresCount = 0 {
        didSet {
            guard oldValue != teamBScoresCount else { return }

            teamBPointsLabel.text = String(teamBScoresCount)
        }
    }
    ...
    func configureView() {
        ...
        let disableInGameUI = UserDefaults.standard.disableInGameUI
        let soloGame = gameManager?.isSolo ?? true
        
        scoresLabel.isHidden = disableInGameUI || !presentedTeamSelector
        
        teamAPointsLabel.isHidden = disableInGameUI || !presentedTeamSelector
        teamBPointsLabel.isHidden = disableInGameUI || soloGame || !presentedTeamSelector
    ...
    func configureARSession() {
        ...
        case .localizingToBoard:
            ...
            if let boardAnchor = gameBoard.anchor {
                ...
                gameBoard.simdScale = float3(repeating: Float(boardAnchor.size.width))
            }
    ...
    func updateGameBoard(frame: ARFrame) {
        ...
        if case .normal = frame.camera.trackingState {
            ...
            if let result = sceneView.hitTest(screenCenter, types: [.estimatedVerticalPlane, .estimatedHorizontalPlane, .existingPlaneUsingExtent]).first {
    ...
    func preloadLevel() {
        ...
        background.async {
            let level = GameLevel.defaultLevel
    ...
    // MARK: - Team logic
    private var presentedTeamSelector = false {
        didSet {
            configureView()
        }
    }

    private func processTeamSelection(_ team: Team) {
        guard let gameManager = gameManager else { return }
        let action = GameAction.joinTeam(team)
        
        gameManager.queueAction(gameAction: action)
        
        if gameManager.isNetworked {
            gameManager.send(gameAction: action)
        }
        os_signpost(.event, log: .render_loop, name: .process_command,
                    "Player trying to join the team: s%", team.description)
        
        sessionState = .gameInProgress
        presentedTeamSelector = true
    }
}
    ...
    func renderer(_ renderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
        ...
        if let gameManager = self.gameManager, gameManager.isInitialized {
            ...
            if let pointOfView = sceneView.pointOfView, let level = selectedLevel, level.placed {
                ...
                let isGrabbingDart = gameManager.isCurrentPlayerGrabbingADart()
                
                DispatchQueue.main.async {
                    if self.sessionState == .gameInProgress {
                        if self.presentedTeamSelector == false {
                            guard let gameManager = self.gameManager else { return }
                            if gameManager.isSolo {
                                self.processTeamSelection(.teamA)
                            } else {
                                let teamPickerVC: TeamPickerViewController = .initWithStoryboard()!
                                self.present(teamPickerVC, animated: true) {
                                    teamPickerVC.delegate = self
                                }
                            }
                            self.presentedTeamSelector = true
                        }
...
extension GameViewController: GameManagerDelegate {
    ...
    func manager(_ manager: GameManager, updated gameState: GameState) {
        DispatchQueue.main.async {
            if self.sessionState == .gameInProgress {
                self.teamAScoresCount = gameState.teamAScores
                self.teamBScoresCount = gameState.teamBScores
            }
        }
    }
}

extension GameViewController: GameStarter {
    private func createGameManager(for session: NetworkSession?) {
        ...
        os_log(.info, "created GameManager")
    }
...
extension GameViewController: TeamPickerViewControllerDelegate {
