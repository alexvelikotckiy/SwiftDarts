...
class PhysicsSyncSceneData {
    ...
    // Thrower sync
    private var throwerList = [CameraThrower]()
    private var throwerDataList = [PhysicsNodeData]()
    ...
    func generateData() -> PhysicsSyncData {
        ...
        // Update Data of throwers in the pool
        for (index, thrower) in throwerList.enumerated() {
            if let data = thrower.generatePhysicsData() {
                throwerDataList[index] = data
            }
        }
        ...
        let packet = PhysicsSyncData(packetNumber: packetNumber, nodeData: nodeDataList,
                                     throwerData: throwerDataList, projectileData: projectileDataList,
    ...
    private func apply(packet: PhysicsSyncData) {
        ...
        throwerDataList = packet.throwerData
    ...
    private func updateObjectsFromData(isHalfway: Bool) {
        ...
        for arrayIndex in 0..<throwerList.count {
            var thrower = throwerList[arrayIndex]
            let nodeData = throwerDataList[arrayIndex]
            
            if nodeData.isAlive {
                if !thrower.isAlive {
                    thrower = delegate.spawnCameraThrower(
                        objectIndex: thrower.index,
                        team: nodeData.team,
                        playerID: nodeData.playerID
                    )
                    throwerList[arrayIndex] = thrower
                }
                
                thrower.apply(physicsData: nodeData, isHalfway: isHalfway)
            } else {
                if thrower.isAlive {
                    delegate.despawnCameraThrower(thrower)
                }
            }
        }
    ...
    func addCameraThrower(_ thrower: CameraThrower) {
        guard let data = thrower.generatePhysicsData() else { return }
        lock.lock() ; defer { lock.unlock() }
        throwerList.append(thrower)
        throwerDataList.append(data)
    }
    
    func replaceCameraThrower(_ thrower: CameraThrower) {
        lock.lock() ; defer { lock.unlock() }
        for (arrayIndex, oldThrower) in throwerList.enumerated() where oldThrower.index == thrower.index {
            throwerList[arrayIndex] = thrower
            return
        }
        fatalError("Cannot find the thrower to replace \(thrower.index)")
    }
