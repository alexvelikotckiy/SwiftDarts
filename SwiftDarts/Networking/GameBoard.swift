...
class GameBoard: SCNNode {
    ...
    static let maximumScale: Float = 0.7
    ...
    /// the default size.
    var preferredSize: CGSize = CGSize(width: 1, height: 1) {
        didSet {
    ...
    override init() {
        ...
        simdScale = float3(repeating: GameBoard.minimumScale)
    ...
    func scale(by factor: Float) {
        ...
        simdScale = float3(repeating: newScale)
    }
    
    func useDefaultScale() {
        let scale = preferredSize.width
        simdScale = float3(repeating: Float(scale))
    }
    ...
    private func scaleToPlane(_ planeAnchor: ARPlaneAnchor) {
        ...
        simdScale = float3(repeating: width)
    ...
    private func performOpenAnimation() {
        ...
        SCNTransaction.animate(duration: GameBoard.animationDuration / 4, animations: {
            ...
            SCNTransaction.animate(duration: GameBoard.animationDuration / 4, animations: {
                SCNTransaction.animationTimingFunction = CAMediaTimingFunction(name: .easeOut)
                self.simdScale = float3(repeating: GameBoard.minimumScale)
            })
        }, completion: {
    ...
    private lazy var fillPlane: SCNNode = {
        ...
        let textureScale = float4x4(scale: float3(40, 40, 1))
